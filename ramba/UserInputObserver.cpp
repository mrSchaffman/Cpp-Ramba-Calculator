...
namespace ui
{
    UserInputObserver::UserInputObserver(client::CommandDispatcher& cd):Observer("CommandEntered"), m_cd(cd)
    {
    }
    void UserInputObserver::notifyImpl(std::shared_ptr<utility::Event> ev)
    {
        auto data = std::dynamic_pointer_cast<UIEvent>(ev);
        if (data)
            m_cd.commandEntered(data->getMessage());
        else
